class LoginManager {
    ...
    initializeElements() {
        // Login form elements
        this.loginFormContainer = document.getElementById('loginForm'); // Container div
        this.loginForm = document.getElementById('emailPasswordForm'); // Actual form
        this.mfaForm = document.getElementById('mfaCodeForm');
        this.mfaContainer = document.getElementById('mfaContainer'); // Main MFA container
        this.mfaCodeEntry = document.getElementById('mfaCodeEntry'); // Code entry section
    ...
    async handleLogin(e) {
        ...
        try {
            ...
            // Get remember device setting from the MFA form if available, otherwise use false
    ...
    async handleMFAVerification(e) {
        ...
        try {
            // Get remember device setting from the checkbox
            const rememberDeviceCheckbox = document.getElementById('rememberDevice');
            const rememberDevice = rememberDeviceCheckbox ? rememberDeviceCheckbox.checked : false;
    ...
    // MFA Helper Methods
    showMFAForm() {
        console.log('Showing MFA form');
        console.log('Login form container:', this.loginFormContainer);
        console.log('MFA container element:', this.mfaContainer);
        console.log('MFA code entry element:', this.mfaCodeEntry);
        console.log('MFA inputs:', this.mfaInputs);
        ...
        // Show MFA container and code entry
        if (this.mfaContainer) {
            this.mfaContainer.style.display = 'block';
            
            // Hide method selection and show code entry directly
            const methodSelection = document.getElementById('mfaMethodSelection');
            if (methodSelection) {
                methodSelection.style.display = 'none';
            }
            
            if (this.mfaCodeEntry) {
                this.mfaCodeEntry.style.display = 'block';
                if (this.mfaInputs && this.mfaInputs.length > 0) {
